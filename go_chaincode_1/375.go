...
func readRecordFile(filename string) (string, error) {
...
func getRecordsFromFile(filename string) ([]Record, error) {
	recordList, err := readRecordFile(filename)
	if err != nil {
		panic(err)
	}
	return deserializeJson(recordList), err
}
...
func (s *SmartContract) InitLedger(ctx contractapi.TransactionContextInterface) error {
	...
	// Method to get new records, from a file

	records, err := getRecordsFromFile("./graph_sample.json")
	if err != nil {
		fmt.Print(err)
	}
	for _, record := range records {
		...
		err = ctx.GetStub().PutState(record.FP, recordJSON)
		if err != nil {
...
func (s *SmartContract) CreateRecord(ctx contractapi.TransactionContextInterface, FP string, BP string, data string) error {
	exists, err := s.RecordExists(ctx, FP)
	if err != nil {
		return err
	}
	if exists {
		return fmt.Errorf("the record %s already exists", FP)
	}
	...
	return ctx.GetStub().PutState(FP, recordJSON)
}
...
func (s *SmartContract) GetRecordbyID(ctx contractapi.TransactionContextInterface, pointer string, tag string) (*Record, error) {
	record, err := getQueryResultForQueryString(ctx, fmt.Sprintf(`{"selector":{"%s":"%s"}}`, tag, pointer))
	if err != nil {
		return nil, fmt.Errorf("failed to read from world state: %v", err)
	}
	if record == nil {
		return nil, fmt.Errorf("the record with %s as %s does not exist", pointer, tag)
	}
	return record[0], nil
}
...
func (s *SmartContract) RecordExists(ctx contractapi.TransactionContextInterface, FP string) (bool, error) {
	recordJSON, err := ctx.GetStub().GetState(FP)
	if err != nil {
