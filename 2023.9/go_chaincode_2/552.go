...
type MockStub struct {
	...
	// flag for cc2cc invocation via InvokeChaincode to dump state on outer tx finish
	// https://github.com/hyperledger-labs/cckit/issues/97
	cc2ccInvocation bool
}
...
func (stub *MockStub) InvokeChaincode(chaincodeName string, args [][]byte, channel string) peer.Response {
	...
	otherStub.cc2ccInvocation = true
...
func (stub *MockStub) Init(iargs ...interface{}) peer.Response {
	args, err := serialize.ArgsToBytes(iargs, stub.Serializer)
	if err != nil {
...
func (stub *MockStub) MockTransactionEnd(uuid string) {
	...
	if !stub.cc2ccInvocation { // skip for inner tx cc2cc calls
		stub.DumpStateBuffer()
		stub.dumpEvents() // events works only for outer stub in Fabric

		// dump buffer to state on outer tx finishing (https://github.com/hyperledger-labs/cckit/issues/97)
		for _, invokableStub := range stub.Invokables {
			invokableStub.DumpStateBuffer()
			invokableStub.cc2ccInvocation = false
		}
...
func (stub *MockStub) Invoke(funcName string, iargs ...interface{}) peer.Response {
	fargs, err := serialize.ArgsToBytes(iargs, stub.Serializer)
	if err != nil {
		return shim.Error(err.Error())
	}
