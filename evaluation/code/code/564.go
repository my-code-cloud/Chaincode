...
func (s *SmartContract) EndAuction(ctx contractapi.TransactionContextInterface, auctionID string) error {
	...
	// Set auction.Price to second highest bid
	// loop through revealed bids and find second highest bid
	secondHighestBid := 0
	for _, bid := range revealedBidMap {
		if bid.Price > secondHighestBid && bid.Price < auction.Price {
			secondHighestBid = bid.Price
		}
	}
	
	auction.Price = secondHighestBid
