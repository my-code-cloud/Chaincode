...
// Create a new logger
var logger = log.ChildLogger(log.RootLogger(), "activity-fabric-getbycompositekey")
...
func (a *Activity) Eval(ctx activity.Context) (done bool, err error) {
	...
	if err != nil {
		logger.Errorf("failed to extract composite key values: %+v\n", err)
		output := &Output{Code: 400, Message: err.Error()}
		ctx.SetOutputObject(output)
		return false, err
	}
	logger.Debugf("Extracted composite key %s and values %+v", keyName, keyValues)
...
func retrievePrivateDataByCompositeKey(ctx activity.Context, ccshim shim.ChaincodeStubInterface, input *Input, name string, values []string) (bool, error) {
	...
	if input.PrivateCollection == "" {
		logger.Error("private collection is not specified\n")
	...
	if err != nil {
		logger.Errorf("failed to retrieve by composite key %s from private collection %s: %+v\n", name, input.PrivateCollection, err)
	...
	if jsonBytes == nil {
		logger.Infof("no data found for composite key %s and value %+v from private collection %s\n", name, values, input.PrivateCollection)
	...
	logger.Debugf("retrieved data from private collection %s: %s\n", input.PrivateCollection, string(jsonBytes))
...
func retrieveByCompositeKey(ctx activity.Context, ccshim shim.ChaincodeStubInterface, input *Input, name string, values []string) (bool, error) {
	...
	if pageSize > 0 {
		if resultIterator, resultMetadata, err = ccshim.GetStateByPartialCompositeKeyWithPagination(name, values, pageSize, bookmark); err != nil {
			logger.Errorf("failed to retrieve by compsite key %s with page size %d: %+v\n", name, pageSize, err)
		...
		if resultIterator, err = ccshim.GetStateByPartialCompositeKey(name, values); err != nil {
			logger.Errorf("failed to retrieve by composite key %s: %+v\n", name, err)
	...
	if jsonBytes == nil {
		logger.Infof("no data found for composite key %s value %+v\n", name, values)
