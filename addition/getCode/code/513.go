...
func (s *SmartContract) initLedger(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	for i := 0; i < len(list); i += 1 {
		// randomIndex := rand.Intn(len(cars))
		carAsBytes, err := json.Marshal(cars[i])
		APIstub.PutState(list[i], carAsBytes)
...
func (S *SmartContract) queryCarsByOwner(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	for i = 0; ownerAndIdResultIterator.HasNext(); i++ {
		...
		fmt.Printf("Found a asset for index : %s asset id : %s val : %s", objectType, compositeKeyParts[0], compositeKeyParts[1])
...
func (s *SmartContract) queryAllCars(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	resultsIterator, err := APIstub.GetStateByRange(list[0], list[len(list)-1])
	if err != nil {
		return shim.Error(err.Error())
	}
...
func (s *SmartContract) restictedMethod(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	if err != nil {
		...
		return shim.Error("Error while retriving attributes")
	}
	if !ok {
		// The client identity does not possess the attribute
		return shim.Error("Client identity doesnot posses the attribute")
	}
