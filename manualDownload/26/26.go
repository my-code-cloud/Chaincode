...
type Land struct {
	...
	Latitude3  string `json:"latitude3"`
	Longitude3 string `json:"longitude3"`
	Latitude4  string `json:"latitude4"`
	Longitude4 string `json:"longitude4"`
...
func (s *SmartContract) createUser(APIstub shim.ChaincodeStubInterface, args []string) sc.Response{
	...
	var user = Users{User: args[0], Amount:args[1]}
...
func (s *SmartContract) createLand(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	var land = Land{TitleName: args[0], Email: args[1], FirstName: args[2], Area: args[3],Address: args[4],ForSale: args[5],Latitude1: args[6],Longitude1: args[7],Latitude2: args[8],Longitude2: args[9],Latitude3: args[10],Longitude3: args[11],Latitude4: args[12],Longitude4: args[13],RequestedBy: args[14],Price: args[15]}

	landAsBytes, _ := json.Marshal(land)
	APIstub.PutState(args[0], landAsBytes)

	indexName := "title~key"
	titleNameIndexKey, err := APIstub.CreateCompositeKey(indexName, []string{land.Email, args[1]})
	if err != nil {
		return shim.Error(err.Error())
	}
	value := []byte{0x00}
