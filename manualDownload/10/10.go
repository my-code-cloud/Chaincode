...
// ===============================================
// putPrivateData - put private data in implicit private data collection
// ===============================================
...
func (t *SimpleChaincode) putPrivateData(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	// check if arguments are passed, because private data must be passed in transient map
	if len(args) != 0 {
		return shim.Error("Incorrect number of arguments. Private marble data must be passed in transient map.")
	}
	...
	// check if the passed data is complete
	if len(marbleInput.Name) == 0 {
		return shim.Error("name field must be a non-empty string")
	}
	if len(marbleInput.Color) == 0 {
	...
	//put private data
	err = stub.PutPrivateData("collectionMarblePrivateDetails2", marbleInput.Name, marblePrivateDetailsBytes)
	if err != nil {
		return shim.Error(err.Error())
	}
	fmt.Println("- end init marble")
