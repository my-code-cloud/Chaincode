...
type Paper struct {
	...
	Testing1    string `json:"testing1"`
	Testing2    string `json:"testing2"`
...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	case "addPerson":
		return s.addPerson(APIstub, args)
	case "addPaper":
		return s.addPaper(APIstub, args)
	case "updatePaper":
		return s.updatePaper(APIstub, args)
	case "updatePaperHash":
		return s.updatePaperHash(APIstub, args)
	case "submitPaper":
		return s.submitPaper(APIstub, args)
	case "getPersonById":
		return s.getPersonById(APIstub, args[0])
	// case "queryAllCars":
...
func (s *SmartContract) initLedger(APIstub shim.ChaincodeStubInterface) sc.Response {
	// person := []Person{
	// 	Person{Name: "a", Password: "a", Type: "a"},
	// 	Person{Name: "b", Password: "b", Type: "b"},
	// 	Person{Name: "c", Password: "c", Type: "c"},
	// }

	// i := 0
	// for i < len(person) {
	// 	carAsBytes, _ := json.Marshal(person[i])
...
func (s *SmartContract) addPerson(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {

	startKey := "Person1"
	endKey := "Person9999999999999999999999999999999999"
	count := 1
	...
	uId := fmt.Sprintf("%s%d", "Person", count)

	var person = Person{Name: args[0], Type: args[1]}

	personAsBytes, _ := json.Marshal(person)
	APIstub.PutState(uId, personAsBytes)

	indexName := "owner~key"
	colorNameIndexKey, err := APIstub.CreateCompositeKey(indexName, []string{person.Name, args[0]})
	if err != nil {
		return shim.Error(err.Error())
	}
	value := []byte{0x00}
...
func (s *SmartContract) addPaper(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {

	startKey := "Paper1"
	endKey := "Paper9999999999999999999999999999999999"
	count := 1
	resultsIterator, err := APIstub.GetStateByRange(startKey, endKey)
	if err != nil {
		return shim.Error(err.Error())
	...
	uId := fmt.Sprintf("%s%d", "Paper", count)

	var paper = Paper{TeacherName: args[0], SubjectName: args[1], StartTime: args[2], EndTime: args[3], Testing: args[4], Testing1: args[5], Testing2: args[6], PaperHash: args[7]}
	paperAsBytes, _ := json.Marshal(paper)
	APIstub.PutState(uId, paperAsBytes)

	indexName := "owner~key"
	colorNameIndexKey, err := APIstub.CreateCompositeKey(indexName, []string{paper.SubjectName, args[0]})
	if err != nil {
		return shim.Error(err.Error())
	}
	value := []byte{0x00}
...
func (s *SmartContract) updatePaper(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {

	//args[0] is requestId, args[1] is fromUser, then args[2] is toUser, then args[3] is date,then args[4] is details
	if len(args) != 6 {
		return shim.Error("Incorrect number of arguments. Expecting 5")
	}

	//split := strings.Split(args[4], "##############################")
	//rsDetails := make([]string, len(split))
	//rsDetails = split

	//rsDetails := []string{args[4]}
	var request = Paper{TeacherName: args[1], SubjectName: args[2], StartTime: args[3], EndTime: args[4], PaperHash: args[5]}

	//updating the request
	requestJSON, _ := json.Marshal(request)
	APIstub.PutState(args[0], requestJSON)

	return shim.Success(requestJSON)
}

func (s *SmartContract) updatePaperHash(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	json.Unmarshal(carAsBytes, &paper)
	paper.PaperHash = args[1]

	carAsBytes, _ = json.Marshal(paper)
	APIstub.PutState(args[0], carAsBytes)

	return shim.Success(carAsBytes)
}

func (s *SmartContract) submitPaper(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {

	startKey := "Submit1"
	endKey := "Submit9999999999999999999999999999999999"
	count := 1
	resultsIterator, err := APIstub.GetStateByRange(startKey, endKey)
	if err != nil {
		return shim.Error(err.Error())
	...
	uId := fmt.Sprintf("%s%d", "Submit", count)
	b, err := strconv.ParseBool(args[4])

	var submit = SubmitPaper{StudentName: args[0], SubjectName: args[1], StartTime: args[2], EndTime: args[3], OnTime: b, PaperHash: args[5]}
	submitAsBytes, _ := json.Marshal(submit)
	APIstub.PutState(uId, submitAsBytes)

	indexName := "owner~key"
	colorNameIndexKey, err := APIstub.CreateCompositeKey(indexName, []string{submit.SubjectName, args[0]})
	if err != nil {
		return shim.Error(err.Error())
	}
	value := []byte{0x00}
	...
	return shim.Success(submitAsBytes)
}

func (s *SmartContract) getPersonById(stub shim.ChaincodeStubInterface, args string) sc.Response {

	fmt.Printf("- getQueryResultForQueryString queryString:\n%s\n", args)

	resultsIterator, err := stub.GetQueryResult(args)
	if err != nil {
		return shim.Error(err.Error())
	}
	defer resultsIterator.Close()
